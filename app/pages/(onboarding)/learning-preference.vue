...
const preferences: Preference[] = [
  {
    ...
    options: [
      { value: '5', label: '5 min', icon: 'i-lucide-zap', gradient: 'from-yellow-400 to-orange-500' },
      { value: '10', label: '10 min', icon: 'i-lucide-flame', gradient: 'from-orange-500 to-red-500' },
      { value: '15', label: '15 min', icon: 'i-lucide-rocket', gradient: 'from-blue-500 to-indigo-600' },
      { value: '30', label: '30+ min', icon: 'i-lucide-trophy', gradient: 'from-purple-500 to-violet-600' }
    ]
  },
  {
    id: 'style',
    ...
    options: [
      { value: 'conversation', label: 'Conversations', icon: 'i-lucide-message-circle', gradient: 'from-cyan-500 to-teal-600' },
      { value: 'structured', label: 'Structured', icon: 'i-lucide-book-open', gradient: 'from-blue-500 to-indigo-600' },
      { value: 'roleplay', label: 'Roleplay', icon: 'i-lucide-drama', gradient: 'from-pink-500 to-rose-600' },
      { value: 'mixed', label: 'Mix', icon: 'i-lucide-layers', gradient: 'from-purple-500 to-violet-600' }
    ]
  },
  {
    id: 'reminder',
    ...
    options: [
      { value: 'morning', label: 'Morning', icon: 'i-lucide-sunrise', gradient: 'from-yellow-400 to-orange-500' },
      { value: 'afternoon', label: 'Afternoon', icon: 'i-lucide-sun', gradient: 'from-orange-500 to-amber-500' },
      { value: 'evening', label: 'Evening', icon: 'i-lucide-sunset', gradient: 'from-purple-500 to-pink-500' },
      { value: 'none', label: 'No reminder', icon: 'i-lucide-bell-off', gradient: 'from-gray-400 to-gray-600' }
    ]
  }
]
...
const isComplete = computed(() => preferences.every(p => answers.value[p.id]))
...
<template>
  <div class="space-y-10">
    ...
    <div class="text-center space-y-3 animate-slide-up">
      <p class="text-sm font-bold text-primary-600 dark:text-primary-400 uppercase tracking-wider">
      ...
      >
        ...
        >
          <span
            ...
            :style="{ color: answers[currentPref.id] === option.value ? 'white' : '' }"
          />
        </div>
    ...
    >
      <div v-if="isComplete" class="p-6 rounded-3xl glass-card border-2 border-primary-500/20">
        <h3 class="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
          <span class="i-lucide-sparkles w-5 h-5 text-primary-500" />
          Your preferences
        </h3>
        <div class="grid gap-3 text-sm">
          <div v-for="pref in preferences" :key="pref.id" class="flex items-center gap-3 p-3 rounded-xl glass-subtle">
            <span class="font-medium text-gray-500 dark:text-gray-400 w-24">{{ pref.category }}:</span>
    ...
    <div class="pt-4">
      ...
      >
        <div v-if="isComplete" class="absolute inset-0 animate-shimmer opacity-30" />
        <span class="relative">Continue</span>
        <span class="relative i-lucide-arrow-right w-5 h-5 transition-transform duration-300 group-hover:translate-x-1" />
      </button>
    </div>
  </div>
</template>
